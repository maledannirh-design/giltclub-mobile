...
export async function renderPrivasi(){
  ...
  content.innerHTML = `
    <div class="account-container">
      ...
      <div class="akun-page-title">Pengaturan Privasi</div>
      ...
      <div class="panel">
        ...
        <div class="akun-section">
          <label>Siapa bisa chat</label>
          <select id="chatPermission">
            <option value="all" ${privacy.chatPermission === "all" ? "selected" : ""}>
              Semua
            </option>
            <option value="followers" ${privacy.chatPermission === "followers" ? "selected" : ""}>
              Followers
            </option>
            <option value="none" ${privacy.chatPermission === "none" ? "selected" : ""}>
              Tidak ada
            </option>
          </select>
        </div>

        <button class="form-submit" id="savePrivacyBtn">
          Simpan Perubahan
        </button>
  ...
  document.getElementById("savePrivacyBtn").onclick = async () => {
    ...
    if(!["all","followers","none"].includes(chatPermission)){
      alert("Pilihan chat tidak valid.");
      return;
    }
